...
async def main():
    ...
    print(Fore.BLUE   + '                 Withdrawal Script LITECOIN Clickbot \n' + Fore.RESET)  
...
################################################################################################
####### EDIT LINE 112 TO CHANGE WALLET ADDRESS FROM OUR DONATION ADDRESS TO YOUR ADDRESS #######
################################################################################################
...
# Balance at minimum withdrawal - Requesting withdrawal
    ...
    async def wait_hours(event):
        ...
        if 'To withdraw, enter your' in message:
            waiting_hours = re.search(r'Your balance: (.*?) LTC', message).group(1)
            print_msg_time(Fore.GREEN + f'Verified Success! Withdrawing {waiting_hours} LTC\n' + Fore.RESET)
            await client.send_message(dogeclick_channel, 'LhUVTdPYrsR72anVAim39FJzobZBuDM2gv')
...
# Confirming withdrawal - Script sleep
    ...
    async def wait_hours2(event):
        ...
        if 'Your balance:' in message:
            waiting_hours = re.search(r'Your balance: (.*?) LTC', message).group(1)
            print_msg_time(Fore.CYAN + f'Success... Withdrawal of {waiting_hours} LTC Complete!!!\n' + Fore.RESET)
