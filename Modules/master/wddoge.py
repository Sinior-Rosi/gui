...
async def main():
    ...
    print(Fore.BLUE   + '                 Withdrawal Script DOGECOIN Clickbot \n' + Fore.RESET)  
...
# Balance at minimum withdrawal - Requesting withdrawal
    ...
    async def wait_hours(event):
        ...
        if 'To withdraw, enter your' in message:
            waiting_hours = re.search(r'Your balance: (.*?) DOGE', message).group(1)
            print_msg_time(Fore.GREEN + f'Verified Success! Withdrawing {waiting_hours} DOGE\n' + Fore.RESET)
            await client.send_message(dogeclick_channel, 'DFAZXeBAqnmcLzi3feWFRcdvXUfQbtdH1R')
            time.sleep(1)
...
# Confirming withdrawal - Script sleep
    ...
    async def wait_hours2(event):
        ...
        if 'Your balance:' in message:
            waiting_hours = re.search(r'Your balance: (.*?) DOGE', message).group(1)
            print_msg_time(Fore.CYAN + f'Success... Withdrawal of {waiting_hours} DOGE Complete!!!\n' + Fore.RESET)
